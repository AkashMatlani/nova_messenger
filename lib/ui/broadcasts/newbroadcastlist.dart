...
class NewBroadcastList extends StatefulWidget {
  NewBroadcastList();

  @override
  NewBroadcastListState createState() {
    return NewBroadcastListState();
  }
}

class NewBroadcastListState extends State<NewBroadcastList> {
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                if (userIdsList.length > 0) {
                  _showCreateBroadCastModal(context);
                } else {
                  showDialog(
                    context: context,
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            SizedBox(height: 5),
            Text(
              "${selectedUsers.length} of ${contactsDetailsInfo.length} selected",
              style: TextStyle(
                  fontSize: 12,
  ...
  Widget mainWidget() {
    return SingleChildScrollView(
        child: contactsDetailsInfo != null
            ? ListView.builder(
  ...
  void _showCreateBroadCastModal(BuildContext context) {
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'Enter broadcast name',
                            ...
                            showDialog(
                              ...
                              builder: (BuildContext context) {
                                return Dialog(
                                  ...
                                  child: CustomToastError(
                                    message1: 'Error',
                                    message2: "Please enter broadcast name",
                                  ),
                                );
                              },
                            );
  ...
  void createBroadCastList() async {
    ...
    broadcastData.name = titleController.text;
    broadcastData.description = "";
    broadcastList.contacts = userIdsList;
    broadcastList.broadcastList = broadcastData;
    CreateBroadcastListResponse broadCastResponse =
        await _api.createShowBroadcastList(broadcastList);
    if (broadCastResponse != null) {
      // TODO: @Ryan Problem here for the pop is it goes back to the action bar, so we need to maybe direct this to broadcast and also refresh
      Navigator.of(context).pop();
      //listGlobalData.add(broadcastList);
      globalSocketService.bindListChannel(broadCastResponse.uuid);
      serviceLocator<ChatViewModel>().listMessages[broadCastResponse.uuid] = [];
      showDialog(
        context: context,
        barrierDismissible: false,
        barrierColor: Colors.transparent,
        builder: (BuildContext context) {
          return Dialog(
            ...
            backgroundColor: Colors.white.withOpacity(0),
            child: CustomToast(
              message1: 'Created new broadcast list',
              message2: null,
            ),
          );
        },
      ...
      Commons.novaFlushBarError(
          context, "Error creating broadcast list. Please try again.");
    }
  }
