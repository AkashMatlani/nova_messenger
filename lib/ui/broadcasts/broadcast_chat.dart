...
import 'package:nova/ui/broadcasts/broadcastinfo.dart';
...
import 'package:nova/ui/widgets/messages_broadcasts_widget.dart';
...
class BroadCastChat extends StatefulWidget {
  ...
  BroadCastChat({this.broadcastData, this.isForwarded, this.forwardedMessage});

  @override
  _BroadCastChatState createState() =>
      _BroadCastChatState(broadcastData: broadcastData);
}
...
class _BroadCastChatState extends State<BroadCastChat>
  ...
  _BroadCastChatState({@required this.broadcastData});
  ...
  String serverURLUPLOAD = dotenv.env['SERVERUpload'];
  var pathRecording = "";
  int replyIndex = 0;
  ...
  void initState() {
    ...
    getUserID();
    inChat = true;
    if (serviceLocator<ChatViewModel>().listMessages[broadcastData.uuid] ==
        null) {
      serviceLocator<ChatViewModel>().listMessages[broadcastData.uuid] = [];
    }
    serviceLocator<ChatViewModel>().setListPeer(broadcastData.uuid);
    linkChannel();
    inChatUuid = broadcastData.uuid;
    _api.getContactsAdmin(broadcastData.uuid);
    super.initState();
    ...
    setUnreadCount();
    getListUUIDs();
    serviceLocator<ChatViewModel>().checkBroadcasts(0);
    // get last typed message //
    textEditingController = TextEditingController(
        text: serviceLocator<ChatViewModel>()
            .messagesLastTyped[broadcastData.uuid]);
  ...
  void markAllMessages() async {
    if (broadcastData.uuid != null && await checkInternet())
      globalSocketService.push(
          id: broadcastData.uuid,
          type: "broadcast",
          event: "mark_broadcasts_as_read");
  }

  void setUnreadCount() async {
    serviceLocator<ChatListViewModel>().mainChatList.forEach((chat) {
      if (chat.list != null) {
        if (chat.list.uuid == widget.broadcastData.uuid) chat.unreadCount = 0;
  ...
  List<Broadcast> _searchResult = [];
  List<Broadcast> messagesSearchChat = [];
  List<String> listAdminUUIDs = [];
  HttpService _api = serviceLocator<HttpService>();

  void getListUUIDs() async {
    AdminContacts contactsAdminResponse =
        await _api.getContactsAdmin(widget.broadcastData.uuid);
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
        onWillPop: () async {
          ...
          inChatUuid = "";
          senderPhoenix = "";
          serviceLocator<ChatViewModel>().senderListPhoenix = "";
                  ...
                  appBar: searchData == true
                      ? AppBar(
                          title: searchTextField(
                              model.listMessages[widget.broadcastData.uuid]),
                          centerTitle: false,
                          elevation: 0,
                          backgroundColor:
                        ...
                        ? Stack(
                            children: [
                              Container(
                                child: Column(
                                  children: <Widget>[
                                    buildListMessage(model.listMessages[
                                        widget.broadcastData.uuid]),
  ...
  Widget buildListMessage(List<Broadcast> messages) {
    return Flexible(
  ...
  Widget searchTextField(List<Broadcast> messages) {
  ...
  Widget buildItem(int index, Broadcast message) {
                        ...
                        children: <Widget>[
                          message.contentType == "text"
                              ...
                              ? myTextMessage(
                                  ...
                                  message.status,
                                  index,
                                  widget.broadcastData.user.uuid,
                                  message)
                              : message.contentType == "image"
                                  // Image/GIF
                                  ? myImageWidget(
  ...
  Widget myVoiceWidget(BuildContext context, CustomClipper clipper, content,
    ...
    int actualIndex = viewModel.listMessages[broadcastData.uuid]
    ...
    return message.hasReplied == false
        ? AnimatedContainer(
            ...
            child: ChatBubble(
              ...
              padding:
                  EdgeInsets.only(top: 12, bottom: 12, left: 15, right: 12),
                      ...
                      Row(
                        ...
                        children: [
                          ProfileCachedNetworkImage(
                              imageUrl: message.avatar, size: 35),
                          Expanded(
                            child: AudioBubble(
                              filepath: content,
        ...
        : AnimatedContainer(
            ...
            color: message.isHighlighted != null && message.isHighlighted
                ? Colors.purple
                : Colors.transparent,
            child: MessagesBroadcastsWidget(
              timestamp: timeStamp,
              read: read,
              index: actualIndex,
        ...
        ? AnimatedContainer(
            ...
            child: Align(
                ...
                child: ChatBubble(
                  ...
                  alignment: Alignment.topRight,
                  padding:
                      EdgeInsets.only(top: 10, bottom: 12, left: 15, right: 12),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      message.ownerUuid != userUuid
                          ? Padding(
                              padding: EdgeInsets.fromLTRB(2, 2, 0, 2),
                              child: Text(
                                message.listUserName ?? globalName,
                                style: TextStyle(
                                    color: appColor,
                                    fontWeight: FontWeight.bold,
                                    fontFamily: normalStyle,
                              ...
                              children: <Widget>[
                                _numeric.hasMatch(content) &&
                                    ...
                                    ? InkWell(
                                        ...
                                        child: contentText(
                                            content, message.senderUuid),
                                      )
                                    : contentText(content, message.senderUuid),
                              ],
                            ),
                          ),
                          message.hasForwarded != null && message.hasForwarded
                              ? SizedBox(
                                  width: 50.0,
                                  height: 10,
                                )
                            ...
                            ClipRRect(
                                ...
                                child: !content.contains(".pdf")
                                    ? Icon(
                                        Icons.note,
                                        size: 20,
                                        color: message.senderUuid == userUuid
                                            ? Colors.white
                                            : Colors.black87,
                                      )
                                    : SvgPicture.asset(
  ...
  Widget myVoiceReplyWidget(BuildContext context, CustomClipper clipper,
      content, star, timeStamp, read, index, id, isPeer, Broadcast message) {
    return Container(
        constraints: BoxConstraints(
          maxWidth: MediaQuery.of(context).size.width * 0.7,
        ),
  ...
  Broadcast getReplyMessageFromUuid(String uuid) {
    ChatViewModel viewModel = serviceLocator<ChatViewModel>();
    var existingReplyMessage = viewModel.listMessages[broadcastData.uuid]
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                          ...
                          child: ForwardingWidget(message.hasForwarded)),
                      message.senderUuid != userUuid
                          ? Padding(
                              padding: EdgeInsets.fromLTRB(2, 2, 0, 2),
                              child: Text(
  ...
  Widget imageWidget() {
    return broadcastData.avatar != ""
        ? Container(
            ...
            child: ClipRRect(
              borderRadius: BorderRadius.circular(30),
              child: customImage(broadcastData.avatar),
            ))
        : Container(
            height: 40,
            width: 40,
  ...
  void replyToMessage(Broadcast message) {
    setState(() {
      replyButton = true;
      replyMessage = message;
    });
  ...
  void _sendMessage() async {
    ...
    serviceLocator<ChatViewModel>().messagesLastTyped[broadcastData.uuid] = "";
    ...
    if (await checkInternet()) {
      try {
        ...
        globalAmplitudeService?.getAnalyticService()?.logEvent('SendBroadcast',
            eventProperties: {'broadcast message sent': true});
      } catch (e) {
        globalAmplitudeService?.sendAmplitudeData(
            'Error on broadcast send:', e.toString(), true);
        await Sentry.captureException(
          e,
        );
      }
      ...
      broadcast.fromUuid = senderPhoenix;
      broadcast.content = textEditingController.text;
      broadcast.contentType = "text";
      broadcast.file = "";
      broadcast.fromUuid = userUuid;
      broadcast.insertedAt = formattedDate;
      broadcast.ownerUuid = widget.broadcastData.user.uuid;
      broadcast.status = "sent";
      broadcast.hasReplied = replyButton;
      broadcast.repliedMessageUuid = replyUuid;
      broadcast.replyIndex = replyIndex;
      broadcast.senderUuid = userUuid;
      broadcast.uuid = broadcastData.uuid;
      broadcast.listUserName = globalName;
      broadcast.clientUuid = uuidClient;
      ...
      serviceLocator<ChatViewModel>().updateListLocalChat(
          widget.broadcastData, broadcastData.uuid, broadcast);
      // Log time outgoing broadcast message //
      globalAmplitudeService?.sendAmplitudeData(
          'Outgoing Broadcast Message Time Stamp',
      ...
      // queueOfflineMessages("BroadcastMessage", broadcastData.uuid,
      //     textEditingController.text, "");
    }
  }
  ...
  void _sendForwardedMessage() async {
    ...
    if (await checkInternet()) {
      ...
      String formattedDate =
          DateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS").format(now);
      broadcast.fromUuid = senderPhoenix;
      broadcast.content = widget.forwardedMessage.content;
      broadcast.contentType = "text";
      broadcast.file = "";
      broadcast.fromUuid = userUuid;
      broadcast.insertedAt = formattedDate;
  ...
  openMessageBox(id, Broadcast message) {
    showModalBottomSheet(
      context: context,
      shape: RoundedRectangleBorder(
                ...
                children: [
                  ...
                  Expanded(
                    child: CircleButtonLongPress(
                      ...
                      onTap: () {
                        Navigator.pop(context);
                        MessagePhoenix messageConverted = MessagePhoenix();
                        messageConverted.fromUuid = message.fromUuid;
                        messageConverted.content = message.content;
                        messageConverted.contentType = message.contentType;
                        if (message.file != null && message.file.isNotEmpty) {
                          messageConverted.file = message.file;
                        } else {
                          messageConverted.file = "";
                        }
                        messageConverted.fromUuid = userUuid;
                        messageConverted.insertedAt = message.insertedAt;
                        messageConverted.status = "sent";
                        messageConverted.hasReplied = message.hasReplied;
                        messageConverted.repliedMessageUuid =
                            message.repliedMessageUuid;
                        messageConverted.replyIndex = message.replyIndex;
                        messageConverted.uuid = message.uuid;
                        messageConverted.clientUuid = message.clientUuid;
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (context) =>
                                  ForwardMessage(messageConverted, "list")),
                        );
                      },
                    ),
                  ),
                      ...
                      child: GestureDetector(
                        onTap: () {
                          globalSocketService.push(
                              id: broadcastData.uuid,
                              type: "broadcast",
                              event: "delete_broadcast",
                              payload: {"uuid": id});
                          serviceLocator<ChatViewModel>()
                              .deleteLocalListMessage();
                          Navigator.pop(context);
                        },
                        child: Column(
                          children: [
  ...
  //This code is for Forwaded Message in BroadCast (Download & upload file to S3)
